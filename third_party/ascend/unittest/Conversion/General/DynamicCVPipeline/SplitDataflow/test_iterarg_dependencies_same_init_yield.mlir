...
// CHECK: hivm.hir.sync_block_set {ssbuffer.analyze_flag_id, ssbuffer.block_id = 5 : i32, ssbuffer.core_type = "CUBE", ssbuffer.transfer_id = 0 : i32}[<CUBE>, <PIPE_M>, <PIPE_MTE3>] flag = [[FLAG_1:[a-z0-9_]+]]
...
// CHECK: hivm.hir.sync_block_set {ssbuffer.analyze_flag_id, ssbuffer.block_id = 5 : i32, ssbuffer.core_type = "VECTOR", ssbuffer.transfer_id = 1 : i32}[<VECTOR>, <PIPE_V>, <PIPE_FIX>] flag = [[FLAG_2:[a-z0-9_]+]]
// CHECK: scf.for {{.*}} iter_args(%[[ARG2:[a-z0-9_]+]] = %[[EXP_2]], %[[ARG3:[a-z0-9_]+]] = %[[MATMUL_7]])

// CHECK: arith.constant {ssbuffer.block_id = 7 : i32, ssbuffer.core_type = "CUBE"} 0 : i32
// CHECK: hivm.hir.sync_block_wait {{.*}} flag = [[FLAG_2]]
// CHECK: hivm.hir.fixpipe {dma_mode = #hivm.dma_mode<nz2nd>, ssbuffer.block_id = 7 : i32, ssbuffer.core_type = "CUBE", ssbuffer.transfer_id = 1 : i32} ins(%[[ARG3]] : tensor<32x32xf32>) outs(%[[ALLOC_2]] : memref<32x32xf32, #hivm.address_space<ub>>)
// CHECK: hivm.hir.sync_block_set {{.*}} flag = [[FLAG_2]]

// CHECK: arith.constant {ssbuffer.block_id = 6 : i32, ssbuffer.core_type = "VECTOR"} 0 : i32
// CHECK: %[[CST_6:[a-z0-9_]+]] = arith.constant
// CHECK: tensor.reshape %[[ARG2]]({{.*}}
// CHECK: tensor.empty()
// CHECK: linalg.transpose
// CHECK: arith.constant
// CHECK: %[[RESHAPE_8:[a-z0-9_]+]] = tensor.reshape
// CHECK: hivm.hir.sync_block_wait {{.*}} flag = [[FLAG_1]]
// CHECK: hivm.hir.copy ins(%[[RESHAPE_8]] : tensor<4x2x16x8xf32>) outs(%[[ALLOC]] : memref<4x2x16x8xf32, #hivm.address_space<cbuf>>) {ssbuffer.block_id = 6 : i32, ssbuffer.core_type = "VECTOR", ssbuffer.transfer_id = 0 : i32}
// CHECK: hivm.hir.sync_block_set {{.*}} flag = [[FLAG_1]]

// CHECK: hivm.hir.sync_block_wait {{.*}} flag = [[FLAG_2]]
// CHECK: memref.memory_space_cast %[[ALLOC_3]] {{{.*}}
// CHECK: %[[TENSOR_12:[a-z0-9_]+]] = bufferization.to_tensor
// CHECK: %[[EXP_13:[a-z0-9_]+]] = math.exp %[[TENSOR_12]] {ssbuffer.block_id = 3 : i32, ssbuffer.core_type = "VECTOR"} : tensor<32x32xf32>
// CHECK: hivm.hir.sync_block_set {{.*}} flag = [[FLAG_2]]
...
// CHECK: %[[MATMUL_16:[a-z0-9_]+]] = linalg.matmul {input_precision = "ieee", ssbuffer.adep, ssbuffer.bdep, ssbuffer.block_id = 4 : i32, ssbuffer.core_type = "CUBE"} ins(%[[TENSOR_15]], %[[TENSOR_15]] : tensor<32x32xf32>, tensor<32x32xf32>)
// CHECK: hivm.hir.sync_block_set {{.*}} flag = [[FLAG_1]]
