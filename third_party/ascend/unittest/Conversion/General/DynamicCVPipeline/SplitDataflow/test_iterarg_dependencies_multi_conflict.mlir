...
// CHECK: %[[[ALLOC_0]:[a-z0-9_]+]] = memref.alloc() {{.*}} : memref<4x2x16x8xf32, #hivm.address_space<cbuf>>
...
// CHECK: scf.for {{.*}} iter_args(%[[ARG_2:[a-z0-9_]+]] = %[[MATMUL_4]])
// CHECK: arith.constant

// CHECK: hivm.hir.sync_block_wait
// CHECK: hivm.hir.fixpipe {{.*}} ins(%[[ARG_2]] : tensor<32x32xf32>) outs(%[ALLOC_1] : memref<32x32xf32, #hivm.address_space<ub>>)
// CHECK: hivm.hir.sync_block_set
...
// CHECK: %[[MATMUL_9:[a-z0-9_]+]] = linalg.matmul {{.*}} ins(%[[ARG_2]], %[[ARG_2]] : tensor<32x32xf32>, tensor<32x32xf32>)

// CHECK: hivm.hir.sync_block_wait
// CHECK: hivm.hir.convert_layout %[[ALLOC_0]]
...
// CHECK: scf.yield {{.*}} %[[TENSOR_11]] : tensor<32x32xf32>
